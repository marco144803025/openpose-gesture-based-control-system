...
def continousKeyboardClick():
    ...
    if gesture_counter == 1 and leftPredictedClassNum != 6:
        ...
        if prevGesture is not None and prevGesture in leftinput:
            controller.release(leftinput[prevGesture])
            print('released', predictedClass[prevGesture])# Release previous key
        if leftPredictedClassNum in leftinput:
            controller.press(leftinput[leftPredictedClassNum]) # Press new key  
        prevGesture=leftPredictedClassNum          
    elif leftPredictedClassNum == 6:
        if prevGesture in leftinput:
            controller.release(leftinput[prevGesture])  # Release previous key
            print('released', predictedClass[prevGesture])
...
#Loading prediction model
    ...
    endFlag = False
    while not endFlag:
        print("Which clicking mode do you prefer? (c for continuous, s for single click): ")
        user_input = input()
...
# Starting OpenPose
    ...
    opWrapper.start()
    id=700
    cap=cv2.VideoCapture("N:\\fyp-dataset\multiview-hand\\annotated_frames\data_3\\0_webcam_4.jpg")
    windowWidth=640
    windowHeight=480
...
# run looping events
    ...
    if time.time() - start_time >= 0.5:
        ...
        if user_input=='s':
        # single click mode for now (require higher accuracy, two frame)
            singleKeyboardClick()
        if user_input=='c':
        # single click a for now (require higher accuracy, two frame)
